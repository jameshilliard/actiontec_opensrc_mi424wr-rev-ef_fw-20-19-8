...
static inline const char *get_version(void)
{
    ...
    const char *cvs_tag = "$Name: BHR2-Release-20-19-8 $";
...
typedef enum {
    LED_ALARM = 0, /* System Mailfaction */
    LED_POWER = 1, /* System Ready */
    LED_WIRELESS = 2, /* Wireless */
    LED_WPS_OK = 3, /* WiFi Protected Setup */
    LED_WPS_FAIL = 4, /* WiFi Protected Setup */
    LED_INET_DOWN = 5, /* Internet Down */
    LED_INET_OK = 6, /* Internet Up */
    LED_MOCA_LAN = 7, /* Coax LAN Link */
    LED_MOCA_WAN = 8, /* Coax WAN Link */
    LED_MOCA_WAN_LIMITED = 9, /* Coax WAN Limited Speed */
    LED_USB = 10, 
    LED_COUNT
} led_t;

static u32 led_to_bitmask_map[] = 
{
    [LED_ALARM] = GPIO_LED | 17, /* System Mailfaction */
    [LED_POWER] = GPIO_LED | 16, /* System Ready */
    [LED_WIRELESS] = GPIO_LED | 8, /* Wireless */
    [LED_WPS_OK] = GPIO_LED | INVERSE_LED | 6, /* WiFi Protected Setup */
    [LED_WPS_FAIL] = GPIO_LED | INVERSE_LED | 0, /* WiFi Protected Setup */
    [LED_INET_DOWN] = GPIO_LED | 13, /* Internet Down */
    [LED_INET_OK] = GPIO_LED | 12, /* Internet Up */
    [LED_MOCA_LAN] = GPIO_LED | 15, /* Coax LAN Link */
    [LED_MOCA_WAN] = GPIO_LED | 14, /* Coax WAN Link */
    [LED_MOCA_WAN_LIMITED] = GPIO_LED | 18, /* Coax WAN Limited Speed */
	[LED_USB] = GPIO_LED | 11, /* USB device */
};
static void gpio_line_config(int pin)
{
    unsigned long long reg_addr;

    if (pin < 16)
    	reg_addr = OCTEON_GPIO_BIT_CFGX(pin);
    else
    	reg_addr = OCTEON_GPIO_XBIT_CFGX(pin);

    octeon_write_csr(reg_addr, 0x0001);
}


static void gpio_line_set(int pin, int val)
{
    octeon_write_csr(val ? OCTEON_GPIO_TX_SET : OCTEON_GPIO_TX_CLR, 1<<pin);
}

static int octeon_leds_op(int led, int state)
{
    u32 led_bitmask;
    u32 pin;
    u32 val;
    ...
    if (led < 0 || led >= LED_COUNT)
        led_bitmask = NO_LED;
    else if (octeon_bootinfo->board_type == CVMX_BOARD_TYPE_MC524WR)
	led_bitmask = led_to_bitmask_map[led];
    else
	led_bitmask = NO_LED;

    if (led_bitmask == NO_LED)
    {
	printk("leds: unknown or unsupported LED: %d\n", led);
	return -1;
    }

    pin = MASK_LED(led_bitmask);
    val = IS_INVERSE_LED(led_bitmask) ? !state : state;

    if (IS_GPIO_LED(led_bitmask))
    {
	gpio_line_config(pin);
	gpio_line_set(pin, val);
    }
    else
    	printk("Unknown type of LED: %#x\n", led_bitmask);
...
//#ifdef ACTION_TEC
    printk("@@@@ Turn off WPS LED when init the KOS\n");
    octeon_leds_op(LED_WPS_OK, 0);
    octeon_leds_op(LED_WPS_FAIL, 0);
//#endif
    ...
    if (region_cfg.s.en)
    {
        ...
        uint64_t width = 1 + region_cfg.s.width;
        printk("Setting flash physical map for %ldMB flash at 0x%08lx %d bit\n", size>>20, base, width * 8);
        physmap_configure(base, size, width, NULL);
    }
#endif
