...
static DEFINE_SPINLOCK(octeon_irq_lock);

extern cvmx_bootinfo_t *octeon_bootinfo;
 
#define IS_MC524WR \
    (octeon_bootinfo->board_type == CVMX_BOARD_TYPE_MC524WR)
 

static uint64_t octeon_irq_core(unsigned int irq)
{
    if (!IS_MC524WR)
        return octeon_get_core_num();
...
static void octeon_unmask_irq(unsigned int irq)
{
    ...
    else if (irq<72)
    {
        ...
        const uint64_t coreid = octeon_irq_core(irq);
