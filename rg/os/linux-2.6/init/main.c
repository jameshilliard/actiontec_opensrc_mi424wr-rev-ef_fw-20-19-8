...
{
#if defined(CONFIG_RG_CRAMFS_IN_FLASH)
	if (do_mount("/dev/mtdblock0", "/mnt/cramfs", "cramfs", MS_RDONLY, 0))
	    printk("Warning: unable to mount cramfs\n");
#elif defined(CONFIG_RG_MAINFS_CRAMFS)
	if (do_mount("cramfs", "/mnt/cramfs", "cramfs_mainfs", MS_RDONLY, 0))
	    printk("Warning: unable to mount cramfs\n");
#else
#error No mainfs filesystem defined
#endif
}
